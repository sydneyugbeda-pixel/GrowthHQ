...
const variants: Record<ButtonVariant, string> = {
  primary:
    "bg-[#2563EB] text-white hover:bg-[#1D4ED8] active:bg-[#7E3E1D] shadow-sm shadow-brand-500/20",
  gradient:
  ...
  outline:
    "border border-[#dfe4ef] text-[#0d1117] bg-white/60 backdrop-blur hover:bg-white hover:border-[#2563EB] dark:border-[#2a3142] dark:text-[#f4f6fb] dark:bg-white/5 dark:hover:bg-white/10",
  destructive:
  ...
  ({ variant = "primary", size = "md", loading, icon, iconRight, className, children, disabled, ...props }, ref) => {
    return (
      <button
        ...
        className={cn(
          "inline-flex items-center justify-center font-semibold transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2563EB] focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer select-none",
          variants[variant],
          sizes[size],
          className
        )}
